...
describe('Intermediate level challenges', () => {
    ...
    describe('findMostExpensiveItem() tests', () => {
        it('Should return a single object', () => {
            expect(findMostExpensiveItem(shoppingBasket)).toHaveProperty(
                'name'
            );
    ...
    describe('settotalPrice() tests', () => {
        ...
        it('Should not modify original array of objects', () => {
            expect(settotalPrice(shoppingBasket)).not.toStrictEqual(
                shoppingBasket
            );
        });
...
xdescribe('cleanCocktailResponseData() tests', () => {
    ...
    it('Should remove old keys', () => {
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('idDrink');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strDrink');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strCategory');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strAlcholohic');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strInstructions');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strIngredients1');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strIngredients2');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strIngredients3');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strIngredients4');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strIngredients5');
        expect(
            cleanCocktailResponseData(mockCocktailData)[0]
        ).not.toHaveProperty('strIngredients6');
    });
    ...
    it('Should add an array for the ingredients', () => {
        expect(
            Array.isArray(
                cleanCocktailResponseData(mockCocktailData)[0].ingredients
            )
        ).toBe(true);
    });
